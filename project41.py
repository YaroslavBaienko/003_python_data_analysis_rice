...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    # get tuples
    res = [(stat[info['playerid']], formula(info, stat))
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    aggr_player_stat = aggregate_by_player_id(
        statistics, info['playerid'], fieldnames)
    ...
    ids_and_stat.sorted(key=lambda x: x[1], reverse=True)
