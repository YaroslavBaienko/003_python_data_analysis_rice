...
def findbynameofcolrow(table, row, col):
    idx = find_col(table=table, col=col)
    idxpython = find_row(table=table, row=row)
    return f"Popularity of {row} in {col}: {table[idxpython][idx]}"


print(findbynameofcolrow(popularity, "C", 1987))
